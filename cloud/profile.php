	...
	<html>
		...
		<body>
			...
			<div id="post_div" class="post_div">
				...
				<div class="h_status" id="h_status">
					<font color="red" class="status">'.$user_post.'</font>
				</div>';
				echo '<br>
				<br>';

				$query = mysql_query("select * from status;") or die(mysql_errno());

				while($row = mysql_fetch_array($query)){
					echo '
					<div id="" class="">
						<font color="red"class="" id="">'.$row['fullname'].'</font>
						<div id="n_status" class="n_status">
							<font color="red" class="" id="">'.$row['status'].'</font>
						</div>
					</div>';
				}
